...
# 데이터 변환: 매수호가와 매도호가를 가로 방향으로 합치기
combined_data_long <- combined_data %>%
  pivot_longer(cols = c(매수호가, 매도호가),
               names_to = "구분",
               values_to = "가격")

# 막대 그래프로 시각화
ggplot(combined_data_long, aes(x = 종목명, y = 가격, fill = 구분)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = 가격), vjust = -1, size = 3,,position = position_dodge(width = 0.9)) +
  labs(title = "매수호가와 매도호가", x = "종목명", y = "가격") +
  theme_minimal()+
  theme(text = element_text(family = "AppleGothic"))
```

\
![매수호가](https://github.com/hyeimii/final/assets/128936045/dc43c87b-d7fb-4908-9891-600d68775c12)

매수호가와 매도호가가 0으로 같은 값인 것이 있다. \
매수호가, 매도호가, 거래량, 거래대금, 전일거래량이 모두 0의 값을 가지는 기업은 "스마트솔루션즈" 이다.\
모두 0인것의 의미는 무엇일까 ?\
모두 0인 주식은 주식시장에서 주목받지 않는 종목으로 간주할 수 있다.\
일반적으로 현재 거래가 없거나 거래정지 상태인 주식을 의미한다.\
